...
public class SPNoticeFragment extends BaseFragment implements
    ...
    public void onBGARefreshLayoutBeginRefreshing(BGARefreshLayout refreshLayout) {
        ...
        map.put("type", Constants.NOTICE_TYPE_SP);
        map.put("offset", mOffset + "");
        map.put("size", "10");
        Xutils.getInstance(mContext).get(API.GET_NOTIFY, map, new Xutils.XCallBack() {
            @Override
            public void onResponse(String result) {
                MainNoticeResult mainNoticeResult = Utils.parseJsonWithGson(result, MainNoticeResult.class);
                if (mainNoticeResult.getData().size() !=0 ){
                    mDatas.clear();
    ...
    public void onRecycleItemClick(int position) {
        if (mDatas.get(position).getExtend() != null || !mDatas.get(position).getExtend().isEmpty()){
            Intent intent = new Intent();
            intent.putExtra("eventId",mDatas.get(position).getExtend());
