...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName
{
    ...
    CGFloat width = frame.size.width / self.numberOfStar;
    for (int i = 0; i < self.numberOfStar; i ++){
        UIImageView *imageView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:imageName]];
        imageView.frame = CGRectMake(1 + i * width , 1, width-2, frame.size.height-2);
...
- (void)changeStarForegroundViewWithPoint:(CGPoint)point
{
    ...
    if (p.x <= 0) {
        ...
        return;
    } else if (0<p.x && p.x<=width) {
        p.x = width;
    } else if (width<p.x && p.x<=width*2) {
        p.x = width * 2;
    } else if (width*2<p.x && p.x<=width*3) {
        p.x = width * 3;
    } else if (width*3<p.x && p.x<=width*4) {
        p.x = width * 4;
    } else if (width*4<p.x) {
        p.x = self.frame.size.width;
    }
