...
- (id)initWithCoder:(NSCoder *)aDecoder
{
    if (self = [super initWithCoder:aDecoder]) {
        ...
        self.scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0.0, 0.0,[UIScreen mainScreen].bounds.size.width, self.bounds.size.height)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(ScrollViewDidScrollToIndex:)]){
        [self.delegate ScrollViewDidScrollToIndex:self.currentPageIndex];
    }
}
