//
//  SPClubMembersDeleteNorViewController.m
//  SportsPage
//
...
@interface SPClubMembersDeleteNorViewController () <UITableViewDelegate,UITableViewDataSource> {
    NSMutableArray *_normalMembersArray;
    NSString *_clubId;
}

@property (weak, nonatomic) IBOutlet UITableView *tableView;
@property (weak, nonatomic) IBOutlet UIButton *deleteMembersButton;

@end

@implementation SPClubMembersDeleteNorViewController
...
- (void)setUpUI {
    [_deleteMembersButton setBackgroundColor:[SPGlobalConfig themeColor]];
    [_deleteMembersButton setTitleColor:[UIColor grayColor] forState:UIControlStateHighlighted];
    _deleteMembersButton.layer.cornerRadius = 5;
    _deleteMembersButton.layer.masksToBounds = true;
}
...
- (IBAction)deleteMembersAction:(UIButton *)sender {
    ...
    [[SPSportBusinessUnit shareInstance] deleteClubMemberBatchWithClubId:_clubId userId:userId memberIds:targetIdStr successful:^(NSString *successsfulString) {
        if ([successsfulString isEqualToString:@"successful"]) {
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                ...
                if ([_delegate respondsToSelector:@selector(finishedDeleteNorAction)]) {
                    [_delegate finishedDeleteNorAction];
                }
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    return [[[_normalMembersArray[section] allValues] objectAtIndex:0] count];
}
